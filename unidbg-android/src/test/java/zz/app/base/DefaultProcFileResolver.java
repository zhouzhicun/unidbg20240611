...
public class DefaultProcFileResolver implements IOResolver<AndroidFileIO> {
    ...
    private FileResult<AndroidFileIO> handleFile(Emulator<AndroidFileIO> emulator, String pathname, int oflags, String fileRoot) {
        ...
        if (pathname.equals(AccessProcFN.proc_cpuinfo)) {
            return IOResolverUtil.createLocalFile(pathname, oflags, fileRoot + LocalProcFN.proc_cpuinfo);
        }

        if (pathname.equals(AccessProcFN.proc_version)) {
            return IOResolverUtil.createLocalFile(pathname, oflags, fileRoot + LocalProcFN.proc_version);
        }
        if (pathname.equals(AccessProcFN.proc_meminfo)) {
            return IOResolverUtil.createLocalFile(pathname, oflags, fileRoot + LocalProcFN.proc_meminfo);
        }
        if (pathname.equals(AccessProcFN.proc_stat)) {
            return IOResolverUtil.createLocalFile(pathname, oflags, fileRoot + LocalProcFN.proc_stat);
        }
        if (pathname.equals(AccessProcFN.proc_self_exe)) {
            return IOResolverUtil.createLocalFile(pathname, oflags, fileRoot + LocalProcFN.proc_self_exe);
        }
        if (pathname.equals(AccessProcFN.proc_asound_cardX_id)) {
            return IOResolverUtil.createLocalFile(pathname, oflags, fileRoot + LocalProcFN.proc_asound_cardX_id);
        }

        if (pathname.equals(AccessProcFN.power_supply_battery_temp)) {
            return IOResolverUtil.createLocalFile(pathname, oflags, fileRoot + LocalProcFN.battery_temp);
        }

        if (pathname.equals(AccessProcFN.power_supply_battery_voltage_now)) {
            return IOResolverUtil.createLocalFile(pathname, oflags, fileRoot + LocalProcFN.battery_voltage_now);
        }
        ...
        if (pathname.equals(temp_proc_pid_status)) {
            ...
            return IOResolverUtil.createByteFile(pathname, oflags, statusDesc);
        }
